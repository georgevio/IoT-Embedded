    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            // Get the client ID - using the config value or MAC address as fallback
                ...
                ESP_LOGI(TAG, "Client ID found: %s", AWS_IOT_CLIENT_ID);
                snprintf(client_id, sizeof(client_id), "%s", AWS_IOT_CLIENT_ID);
            #else
                ESP_LOGI(TAG, "Client ID not found, will use MAC Address");
                ...
                ESP_LOGI(TAG, "Client ID is %s", client_id);
            #endif
            
            // Format initialization message with client_id inline
            snprintf(init_message, sizeof(init_message), 
            "{\"%s is UP at\": %llu}", client_id, (unsigned long long)(esp_timer_get_time() / 1000));            

            // Log the initialization message
            ESP_LOGI(TAG, "Sending: %s", init_message);

            // Publish to MQTT using the client ID as the topic
            mqtt_publish_message(client, client_id, init_message, 1, 0);
...
esp_mqtt_client_handle_t mqtt_aws_init(void)
{
    ...
    ESP_LOGI(TAG, "Root CA Preview: %.16s", _binary_AmazonRootCA1_pem_start);
